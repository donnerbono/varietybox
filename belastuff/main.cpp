...
std::vector<Display> gDisplays = {
	// use `-1` as the last value to indicate that the display is not behind a mux, or a number between 0 and 7 for its muxed channel number
        ...
        {U8G2LinuxI2C(U8G2_R0, gI2cBus, 0x3c, u8g2_Setup_ssd1306_i2c_128x64_noname_f), -1},
          {U8G2LinuxI2C(U8G2_R0, gI2cBus, 0x3c, u8g2_Setup_ssd1306_128x64_noname_1), -1},
...
//	u8g2.clearBuffer();
	...
	// Donnerbono Variety box bits start here....
	}  else if (msg.match("/clearscreen"))
	{u8g2.clearBuffer();}
	
	
		else if (msg.match("/line"))
	...
	else if (msg.match("/myfill"))
	{
		u8g2_uint_t number1, number2, number3, number4;
		if(args.popNumber(number1).popNumber(number2).popNumber(number3).popNumber(number4).isOkNoMoreArgs())
		{
			printf("received /number %d\n", number1);
			u8g2.setDrawColor(0);
			u8g2.drawBox(number1, number2, number3, number4);
			u8g2.setDrawColor(1);
		}
		else
			error = kWrongArguments;
	  ...
	  else if (msg.match("/mynumbers"))
	{
		u8g2_uint_t number1, number2, number3;
		if(args.popNumber(number1).popNumber(number2).popNumber(number3).isOkNoMoreArgs())
		{
			printf("received /number %d\n", number3);
			u8g2.setFont(u8g2_font_7x13_tf);
			u8g2.drawUTF8(number1, number2, std::to_string(number3).c_str());
		}
		else
			error = kWrongArguments;
	} else if (msg.match("/mytext"))
	{
		std::string text1;
		u8g2_uint_t number1, number2;
			if(!args.popNumber(number1).popNumber(number2).popStr(text1).isOkNoMoreArgs())
			error = kWrongArguments;
		else {
			const char *ctrStr1 = text1.c_str();
			printf("received /mytext number %d number %d string %s/n", number1, number2, ctrStr1);
			u8g2.setFont(u8g2_font_7x13_tf);
		//	u8g2.setFontRefHeightText();
			u8g2.drawUTF8(number1, number2, ctrStr1);
			u8g2.drawStr(number1, number2, ctrStr1);
			  }
	}
	
	// ... Donnerbono Variety Box bits end here
